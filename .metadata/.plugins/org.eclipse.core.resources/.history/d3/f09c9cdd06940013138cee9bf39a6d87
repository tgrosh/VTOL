...
public class ZoneObjective extends Objective {	
	...
	public ZoneObjective(LanderScene scene, int id, int objectiveZoneID, String description, IObjectiveListener listener) {
		super(scene, id, description, listener);		
		this.objectiveZoneID = objectiveZoneID;
		
		Load();
	}
	
	public ZoneObjective(LanderScene scene, int id, int objectiveZoneID, String description, int prerequisiteID, IObjectiveListener listener) {
		super(scene, id, description, prerequisiteID, listener);		
		this.objectiveZoneID = objectiveZoneID;
	...
	private void Load() {
		scene.addListener(new ILanderSceneListener() {			
			...
			public void onObjectiveZoneEnter(ObjectiveZone objZone) {
				...
				if (objectiveZoneID == objZone.getId()){
